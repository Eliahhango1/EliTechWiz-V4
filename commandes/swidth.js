...
zokou({
  ...
  try {
    ...
    const cap = `*Screenshot taken by EliTechWiz-V4`;
...
zokou({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by EliTechWiz-V4`;
...
zokou({
  ...
  try {
    ...
    message.sendMessage(user, {
      'image': { 'url': imageUrl },
      'caption': "*powered by EliTechWiz-V4"
    }, { 'quoted': ms });
  } catch (error) {
...
zokou({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and EliTechWiz-V4 will extract the text for you.");
    }
...
zokou({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and EliTechWiz-V4 will generate it.");
    }
    const prompt = args.join(" ");
